										...
										?>
											...
											<?php  $seller_id=$cart->created_by?>
												...
												<div class="pull-right">
														...
														<p>Seller location is not present</p> 
														@elseif(count($items['address']))
														...
														@if($priority_info['result']=="1")
															...
															<div class="radio">
															  <label><input type="radio" name="shipping_type[{{$seller_id}}]" value="0.00_free" class="shipping_amount" data-seller-id="{{$seller_id}}" data-type='free' @if($items['type']=="free") checked="checked" @endif>Free shipping</label> <span class="shiping_amount">$0.00</span>
															</div>
															@endif
														<div class="radio">
														
														  <label><input type="radio" name="shipping_type[{{$seller_id}}]" value="{{$priority_info['msg']['rate']}}_priority" class="shipping_amount" data-seller-id="{{$seller_id}}" data-type='priority' data-shipping-days="{{$priority_info['msg']['MailService']}}">Priority shipping</label> <span class="shiping_amount">${{number_format($priority_info['msg']['rate'], 2, '.', ',')}}</span>
														</div>
														<div class="radio">
														  <label><input type="radio" name="shipping_type[{{$seller_id}}]" value="{{$express_info['msg']['rate']}}_express" class="shipping_amount" data-seller-id="{{$seller_id}}" data-type='express' data-shipping-days="{{$express_info['msg']['MailService']}}">Express shipping</label> <span class="shiping_amount">${{number_format($express_info['msg']['rate'], 2, '.', ',')}}</span>
													</div>
													<span class="error" id="sipping_{{$seller_id}}"></span>
													@endif
													@if($priority_info['result']=="0" && $priority_info['error_code']=="-2147219498")
													<span>Seller location is not present</span> 
													@endif
													@if($priority_info['result']=="0" && $priority_info['error_code']!="-2147219498")
													{{$priority_info['msg']}}
													@endif
								...
								<div class="check_out_page_right">
									<div class="order_summery">
										<div class="well">
											...
											@if(!empty($data['dis_count'])) <p class="sub-all"><span>Coupon code: </span> <span class="sub-price coupan-p" data-coupan="{{$data['dis_total']}}">-${{$data['dis_total']}} <em>({{$data['dis_count']}} Items)</em></span></p>@endif
											...
											<p class="sub-all total_price"><span>Total: </span> <span class="sub-price">@if(!empty($data['basic']['dis_count']))${{number_format($data['basic']['basic'][0]->total-$data['basic']['dis_total']-$data['basic']['basic'][0]->store_credits, 2, '.', ',')}} @else $<span class="total-amount">{{number_format($data['basic']['basic'][0]->total-$data['basic']['basic'][0]->store_credits, 2, '.', ',')}}</span>@endif </span></p>
