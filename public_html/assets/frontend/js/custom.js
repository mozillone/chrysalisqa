$(function(){
 ...
 $("body").on("click", ".main_cat_url_class > a", function(event){
    ...
    window.location.href = main_cat_url;
 });
	...
	$("#signup_pop1").validate({
            rules: {
                ...
                email:{
                 ...
                 cpassword:{
                 	required:true,equalTo:"#popup_password"
                 }
                },
                 messages:{
                 	email:{
   ...
   if ($(window).width() < 1024) {
    ...
    $(document).on('click','.signup_popup', function(){
    	  $('#loginpopup')[0].reset();
   		  ...
   		  $('.lgn').addClass('active in');
          $('.sign').removeClass('active in');
          $('.fpswd').removeClass('active in');
   		 $('#signup_popup').modal('show');
        });
         $(document).on('click','.first_active', function(){
          $('.lgn').addClass('active in');
          $('.sign').removeClass('active in');
          $('.fpswd').removeClass('active in');
     ...
     $(document).on('click mouseover','.mini-cart', function(){
			...
			success: function(response){
				...
				$.each(response['basic'],function(i,value){
					...
					cart+='<div class="col-md-12 col-sm-12 col-xs-12"><div class=""><div class="media-left"><img src='+src+' class="media-object" ></div><div class="media-body"><h4 class="media-heading"><a href="/product'+value.url_key+'">'+value.costume_name+'</a></h4><p>Condition : '+capitalizationCondition(value.condition)+'</p><p class="sizess">Size : '+capitalization(value.size)+'</p></div></div></div>'
				});
					...
					if (jQuery(window).width() < 767) 
{
	...
	jQuery(".bxslider-strt").insertBefore(".priceview_rm");
	jQuery(".bxslider-strt").insertAfter(".mobile_list_view");
		jQuery(".single_view_details").insertAfter(".viewBtn_rm");
jQuery(".col-md-6.col-sm-6.col-xs-12.video_dv").insertAfter(".col-md-6.col-sm-6.col-xs-12.video_cnt");
jQuery(".col-md-6.col-sm-6.col-xs-12.video_cnt_top").insertBefore(".col-md-6.col-sm-6.col-xs-12.video_btm");
jQuery(".col-md-6.col-sm-6.col-xs-12.submission-form.contact-page").insertBefore(".col-md-6.col-sm-6.col-xs-12.cnt_sprt_map");
}
...
function toggleIcon(e) {
	$(e.target)
        .prev('.panel-group-mobile-cms')
        .find(".more-less")
        .toggleClass('glyphicon-plus glyphicon-minus');
}
$('.panel-group-mobile-cms').on('hidden.bs.collapse', toggleIcon);
$('.panel-group-mobile-cms').on('shown.bs.collapse', toggleIcon);





    $(".mb-searchs").click(function(){
        $(".mble_srch-div").toggle();
...
$('.main_menu ul.nav li.dropdown').hover(function() {
  $(this).find('.dropdown-menu').stop(true, true).delay(100).fadeIn(300);
}, function() {
  $(this).find('.dropdown-menu').stop(true, true).delay(100).fadeOut(300);
});

// $(document).on('click','.ft-logo,.quick_links ,.app_img',function(){
// 	if($(this).hasClass('active')){
// 		$(this).find('.shw-msg').show().not().hide();
// 	}else{
// 		$(this).find('.shw-msg').hide().not().show();
// 	}
// });
...
$('#submit').on('click',function(a){
			...
			if(subscribe_email==""){
				...
				$('#subscribeerror').html('Enter Email');
				str=false;
			}
			if (subscribe_email!= '' && !emailpattern.test(subscribe_email)) {
				$('#subscribe_email').css('border', '1px solid red');
				$('#subscribeerror').html('Enter Valid Email');
				str=false;
			}
			if(str==true){
				$.ajax({
                   type: "POST",
                    ...
                    success: function (data) {
                        ...
                        Lobibox.notify.closeAll();
                                ...
                                size: 'mini',
                                title: 'Newsletter',
                                msg: "Thanks for signing up for our newsletter! Please check your inbox to confirm your email address.",
                                });
                        }
                        if(data.code=="204"){
                            Lobibox.notify.closeAll();
                        ...
                        Lobibox.notify.closeAll();
                                ...
                                size: 'mini',
                                title: 'Newsletter',
                                 msg: "We already have you on the mailing list.",
                                });
                        }
                    }
                });
...
$('#search_show').on('click touchstart',function(){

      $('.form.mble_srch-div').show();
  event.stopPropagation();
});

   $('.mb-searchs').on('click',function(){
  $('.form.mble_srch-div').show();
  event.stopPropagation();
});
...
$('.respnsive-ser-rm').click(function() {
