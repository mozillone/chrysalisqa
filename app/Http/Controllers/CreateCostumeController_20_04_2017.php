...
class CreateCostumeController  extends Controller {
	...
	//&& $description!='' && $funfact!='' && $faq!=''){
	   ...
	   $sku_num=DB::table('costumes')->max('sku_no');
		...
		public function Draganddropfrontimage(Request $request){
			$file_name = str_random(10).'.'.$request->userImage->getClientOriginalExtension(); 
            ...
            $request['userImage']->move($source_image_path, $file_name);
			...
			//print_r(public_path(('costumers_images').'/'.$file_name));die;
	 		return $file_name;
	}

	public function Costumedescription(Request $request){
	...
	*/
	...
	public function Costumepricing(Request $request){
		$session_costume_id = Session::get('session_costume_id');
		//echo "<pre>";print_r($request->all());die;
		if (isset($session_costume_id) && !empty($session_costume_id)) {
			$costume_id = $session_costume_id;
		}else{
			return "error";
		}
	...
	*/
	...
	public function Costumepreferences(Request $request){
		echo "<pre>";
		print_r($request->all());
		die;
